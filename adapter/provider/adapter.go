...
func (p *myProviderAdapter) CheckOutbounds(ctx context.Context, force bool) {
	p.Healthcheck(ctx, p.healthcheckUrl, force)
...
func (p *myProviderAdapter) InterfaceUpdated(ctx context.Context) {
	if !p.enableHealthcheck {
		return
	}
	p.CheckOutbounds(ctx, true)
}
