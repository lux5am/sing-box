...
func newHysteriaClashParser(proxy map[string]any) (option.Outbound, error) {
	...
	if upRaw, exists := proxy["up"]; exists {
		...
		case string:
			var ups byteformats.NetworkBytesCompat
			if err := json.Unmarshal([]byte(up), &ups); err != nil {
				return outbound, E.New("invalid hysteria up value")
			}
			options.Up = &ups
		case int:
			options.UpMbps = up
	...
	if downRaw, exists := proxy["down"]; exists {
		...
		case string:
			var downs byteformats.NetworkBytesCompat
			if err := json.Unmarshal([]byte(down), &downs); err != nil {
				return outbound, E.New("invalid hysteria down value")
			}
			options.Down = &downs
		case int:
