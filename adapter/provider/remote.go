...
func NewRemoteProvider(ctx context.Context, manager *Manager, router adapter.Router, options option.OutboundProvider, path string) (*RemoteProvider, error) {
	...
	if downloadInterval < C.DefaultDownloadInterval {
		downloadInterval = C.DefaultDownloadInterval
	}
...
func (p *RemoteProvider) Start() error {
	p.healchcheckHistory = service.PtrFromContext[urltest.HistoryStorage](p.ctx)
	if p.healchcheckHistory == nil {
