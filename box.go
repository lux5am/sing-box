...
func (s *Box) preStart() error {
	...
	err = adapter.Start(s.ctx, s.logger, adapter.StartStateInitialize, s.network, s.dnsTransport, s.dnsRouter, s.connection, s.router, s.provider, s.outbound, s.inbound, s.endpoint, s.service, s.certificateProvider)
	if err != nil {
		return err
	}
	err = adapter.Start(s.ctx, s.logger, adapter.StartStateStart, s.provider, s.outbound, s.dnsTransport, s.network, s.connection)
	if err != nil {
		return err
	}
