...
func getGroupDelay(server *Server) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		} else if fallbackGroup, isFallbackGroup := outboundGroup.(adapter.FallbackGroup); isFallbackGroup {
			result, err = fallbackGroup.URLTest(ctx)
		} else {
			outbounds := common.FilterNotNil(common.Map(outboundGroup.All(), func(it string) adapter.Outbound {
				itOutbound, _ := server.outbound.OutboundWithProvider(it)
				return itOutbound
			}))
