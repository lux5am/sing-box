...
var (
	...
	_ adapter.FallbackGroup           = (*Fallback)(nil)
...
func (s *Fallback) References() []string {
	...
	if selected := group.selectedOutboundTCP.Load(); selected != nil {
		references = append(references, selected.Tag())
	}
	if selected := group.selectedOutboundUDP.Load(); selected != nil {
		references = append(references, selected.Tag())
	}
	return references
}
...
func (g *FallbackGroup) Select(network string) (adapter.Outbound, bool) {
	...
	if minOutbound != nil && common.Contains(minOutbound.Network(), network) {
		if history := g.history.LoadURLTestHistory(RealTag(minOutbound, network)); history != nil {
